...
<div class="page-body">
    <div class="container-xl">
        ...
        <form action="{{ route('homepage.udpate', ['sec' => $section, 'lang' => $language->id, 'id' => $data->id]) }}" method="POST" id="homeForm">
            @method('put')
            @csrf
            @if ($section == 1)
                <h2>{{ $data->header }}</h2>
                <div class="row mb-3">
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control @error('header') is-invalid @enderror" name="header" value="{{ old('header', $data->header) }}" placeholder="General Secretariat Ayude for A New Day">
                        @error('header')
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        ...
                        <textarea class="form-control  @error('content') is-invalid @enderror" name="content" rows="6" placeholder="Content..">{{ old('content', $data->content) }}</textarea>
                        @error('content')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
