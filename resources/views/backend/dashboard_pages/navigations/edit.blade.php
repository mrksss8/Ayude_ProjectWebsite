...
<div class="container-xl">
    <form action="{{ route('navigation.update', ['id' => $nav->id, 'type' => $type]) }}" method="post" id="modalReportForm">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="nav_name" value="{{ $nav->nav_name }}" id="navInput">
        </div>
        <div class="mb-3" id="languageSelect">
            <label class="form-label">Language</label>
            <select class="form-select" id="langSelect" name="language">
                <option value="{{ $nav->lang($nav->language_id)->id }}" id="{{ "select".$nav->lang($nav->language_id)->language }}">{{ $nav->lang($nav->language_id)->language }}</option>
            </select>
        </div>
    </form>
</div>
